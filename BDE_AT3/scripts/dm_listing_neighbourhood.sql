...
SELECT
    ...
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY l.price) OVER (PARTITION BY l.neighbourhood, l.month_year) AS median_price_active_listings,
    ...
    (active_listings - LAG(active_listings) OVER (PARTITION BY l.neighbourhood ORDER BY l.month_year)) / NULLIF(LAG(active_listings) OVER (PARTITION BY l.neighbourhood ORDER BY l.month_year), 0) * 100 AS active_listings_percentage_change,
    (inactive_listings - LAG(inactive_listings) OVER (PARTITION BY l.neighbourhood ORDER BY l.month_year)) / NULLIF(LAG(inactive_listings) OVER (PARTITION BY l.neighbourhood ORDER BY l.month_year), 0) * 100 AS inactive_listings_percentage_change,
